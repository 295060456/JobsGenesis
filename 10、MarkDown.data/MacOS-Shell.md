...
- [Shell实用代码块](#shell实用代码块)
  ...
  - [写文件](#写文件)
    - [接受键盘输入，向文件写一段内容](#接受键盘输入向文件写一段内容)
    - [向文件写一行内容](#向文件写一行内容)
  - [在列表里面（gem list --local）搜寻（grep）文本（cocoapods）](#在列表里面gem-list---local搜寻grep文本cocoapods)
  - [获取当前用户信息](#获取当前用户信息)
    - [终端获取 `${HOME}`](#终端获取-home)
    - [shell 脚本获取](#shell-脚本获取)
    - [1、打印出当前的用户名（注意：这里是双引号）](#1打印出当前的用户名注意这里是双引号)
    - [2、打印出当前的用户进程ID](#2打印出当前的用户进程id)
    - [3、打印出当前用户的详细信息（排列板式不一）](#3打印出当前用户的详细信息排列板式不一)
    - [4、打印当前终端所指向的路径](#4打印当前终端所指向的路径)
    - [5、打印当前执行的脚本文件的父目录](#5打印当前执行的脚本文件的父目录)
    - [6、参数的获取](#6参数的获取)
      - [6.1、假设执行 `./test.sh a b c` 这样一个命令，则可以使用下面的参数来获取一些值：](#61假设执行-testsh-a-b-c-这样一个命令则可以使用下面的参数来获取一些值)
      - [6.2、举例说明 `"$*"` 和 `"$@"` 的差异](#62举例说明--和--的差异)
      - [6.3、获取位置参数的个数](#63获取位置参数的个数)
  - [获取文件的路径、文件名、后缀、父级文件夹名等](#获取文件的路径文件名后缀父级文件夹名等)
    - [1、提取文件全名](#1提取文件全名)
    - [2、提取后缀](#2提取后缀)
    - [3、提取出我们需要的文件所在的目录路径](#3提取出我们需要的文件所在的目录路径)
    - [4、【活学活用】提取我们需要的文件所在路径的父文件夹名字](#4活学活用提取我们需要的文件所在路径的父文件夹名字)
    - [5、同级建立同名目录](#5同级建立同名目录)
    - [6、总结](#6总结)
  - [获取目录下所有文件夹的名称并输出](#获取目录下所有文件夹的名称并输出)
  - [获取桌面路径](#获取桌面路径)
  - [其他](#其他)
